...
/*setInterval("flushmsg();", timeout);*/

function deleteAll(formid){
	var length =$(".deleteids:checked").length;
	if(length>0){
		if(confirm('确定删除这'+length+'条记录？')){
			$("#"+formid).submit();
		}
	}else{
		alert("没有要删除的记录");
...
function selectAll(e){
	if($(e).attr("checked")=="checked"){
		$(".deleteids").attr("checked","checked");
	}else{
		$(".deleteids").removeAttr("checked");
	}
}
...
$(function($) {
	$(window).scroll(function(){
		$('.msg').css({top:(boxheight + $(document).scrollTop())+"px"});
		
		$('#debug').val( 'scrollTop:'+$(document).scrollTop()+' hight:'+h+' boxheight:'+boxheight );
	}); 
	$('.msg').css({top:(h)+"px",opacity:"0"});
	$(".onerow").click(function(){
		var $check = $(this).parent().find(".deleteids").eq(0);
...
function flushmsg(){
	...
	isopen = 1;
	$.post('/index.php?a=flushmsg&m=Index', {}, function(v){
		if( v.status == 1 ){
			$('.msg > a').attr('href', v.data.url);
			$('.msg > a').html(v.data.title);
			$('.msg > p').html(v.data.content);
			boxheight = h - 150;
			$('.msg').show();
			$('.msg').animate({top: (boxheight + $(document).scrollTop())+"px",opacity:"1"}, "slow");
		}
	}, "json");
...
function msg_close(){
	$('#debug').val('close ');
	$.post('/index.php?a=user_close_pop&m=Index', {}, function(v){
		if( v.status == 1 ){
			$('.msg > a').attr('href', v.data.url);
			$('.msg > a').html(v.data.title);
			$('.msg > p').html(v.data.content);
			boxheight = h;
			$('.msg').animate({top:(boxheight + $(document).scrollTop())+"px",opacity:"0"}, "slow", function(){$(this).hide(); isopen = 0;});
			
		}else{
			alert('关闭出错，请重试或联系管理员！');
		}
	}, "json");
}
...
ui = window.ui ||{
	...
	emotions:function(o){
		...
		this.html = '<div class="talkPop alL" id="emotions" style="*padding-top:20px;">'
				 ...
				 + '<img src="/public/themes/classic/images/zw_img.gif" style="margin-left: 10px; position: absolute;" class="talkPop_arrow"></div>'
