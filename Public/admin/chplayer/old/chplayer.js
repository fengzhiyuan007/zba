	...
	chplayer.prototype = {
		...
		interFace: function() {
			...
			*/
			...
			var controlBarBgID = 'controlbgbar' + randomS, //控制栏背景
				...
				ondanmuID = 'ondanmu' + randomS,//显示弹幕
				offdanmuID = 'offdanmu' + randomS,//关闭弹幕
			...
			html += '<div class="' + ondanmuID + '"></div>';//显示弹幕按钮
			html += '<div class="' + offdanmuID + '"></div>'; //关闭弹幕按钮
			...
			this.CB = {
				...
				ondanmu: this.getByElement(ondanmuID,pd),
				offdanmu: this.getByElement(offdanmuID,pd)
			};
			this.buttonWidth = {
				play: bWidth,
				full: bWidth,
			...
			this.css(ondanmuID,{
				...
				top:"5px",
				right:"220px",
				backgroundImage:"url(lmLive/icon/danm_on.png)",
			...
			this.css(offdanmuID,{
				...
				top:"5px",
				right:"220px",
				backgroundImage:"url(lmLive/icon/danm_off.png)",
				backgroundSize:'cover',
				zIndex:"99",
				cursor:'pointer'
			})
		...
		*/
		addButtonEvent: function() {
			var thisTemp = this;
			//定义打开弹幕按钮单击事件
			var offdanmuClick = function(){
				thisTemp.offdanmu();
				// console.log('关闭弹幕')
				// thisTemp.css(thisTemp.CB['offdanmu'], 'display', 'block');//这里要使用thisTemp,this指向问题
				// thisTemp.css(thisTemp.CB['ondanmu'], 'display', 'none');
			}
			this.addListener('click',offdanmuClick,this.CB['ondanmu']);
			//定义关闭弹幕按钮单击事件
			var ondanmuClick = function(){
				thisTemp.ondanmu();
				// console.log("显示弹幕");
				// thisTemp.css(thisTemp.CB['ondanmu'], 'display', 'block');
				// thisTemp.css(thisTemp.CB['offdanmu'], 'display', 'none');
			}
			this.addListener('click',ondanmuClick,this.CB['offdanmu']);
			//定义按钮的单击事件
		...
		*/
		judgeIsLive: function() {
			...
			if(!this.vars['live']) {
				...
				// this.timerTime = new timer(1000, timeFun);//如果是直播，不让显示时间点
				//timerTime.start();
			}
			this.definition();
		},
		...
		*/
		...
		/*	
			...
			显示弹幕
		*/
		ondanmu: function(fun){
			console.log("显示弹幕");
			this.barrage = true;
			console.log(this.barrage);
			this.css(this.CB['ondanmu'], 'display', 'block');
			this.css(this.CB['offdanmu'], 'display', 'none');
		},
		/*	
			接口函数
			关闭弹幕
		*/
		offdanmu: function(fun){
			console.log('关闭弹幕');
			this.barrage = false,
			console.log(this.barrage);
			this.css(this.CB['offdanmu'], 'display', 'block');
			this.css(this.CB['ondanmu'], 'display', 'none');
		},
		/*	接口函数
			在播放和暂停之间切换
		*/
