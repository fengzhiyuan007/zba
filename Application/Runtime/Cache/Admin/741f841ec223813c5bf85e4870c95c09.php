...
function getnums(){
    ...
    window.location.href="/Admin/User/del_user?nums="+num+"&start="+start+"&end="+end+"&username="+username;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
    ...
    <td align="center" class="onerow"><?php echo (date("Y-m-d H:i",$l["del_time"])); ?></td>
  <td align="center">
                  <a href="javascript:;"  onclick="restore(<?php echo ($l["user_id"]); ?>);" >恢复</a>&nbsp;&nbsp;|&nbsp;&nbsp;
...
function del(kid){
    ...
    $.post("<?php echo U('del_true');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('del_user');?>");
        }else{
            alert('删除失败！');
        }
    });
...
function restore(kid){
    ...
    if(!confirm('确定恢复？'))
        return false;
    $.post("<?php echo U('restore');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('恢复成功！');
            location.reload("<?php echo U('del_user');?>");
        }else{
