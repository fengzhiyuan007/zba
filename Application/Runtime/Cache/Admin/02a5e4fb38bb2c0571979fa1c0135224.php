...
function getnums(){
    var num = $("#nus").val();
    window.location.href="/Admin/Base/approve_rule?nums="+num;
}
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/Base/toadd_approve_rule">添加</a></span></div>
...
<tr class="tabTitleMain">
    ...
    <td width="15%" align="center">等级</td>
    <td width="15%" align="center">名称</td>
    <td width="15%" align="center">图标</td>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["approve_rule_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    <td align="center" class="onerow"><?php echo ($l["grade_start"]); ?>~<?php echo ($l["grade_end"]); ?></td>
    <td align="center" class="onerow"><?php echo ($l["name"]); ?></td>
    <td align="center" class="onerow"><img src="<?php echo ($l["grade_img"]); ?>" style="width: 60px;height: 60px"></td>
    <td align="center" class="onerow"><?php echo (date("Y-m-d H:i",$l["intime"])); ?></td>

  <td align="center">
                  <a href="<?php echo U('toadd_approve_rule',array('id'=>$l['approve_rule_id']));?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                  <a href="javascript:;"  onclick="del(<?php echo ($l["approve_rule_id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_approve_rule');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('approve_rule_id');?>");
        }else{
            alert('删除失败！');
        }
    });
