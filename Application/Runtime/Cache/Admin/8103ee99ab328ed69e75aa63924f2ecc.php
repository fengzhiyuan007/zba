...
function getnums(){
    ...
    window.location.href="/Admin/User/fans?nums="+num+"&start="+start+"&end="+end+"&username="+username;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/User/toadd_fans">添加</a></span></div>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  ...
  <td align="center">
                  <a href="<?php echo U('toadd_fans',array('id'=>$l['user_id']));?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
...
function del(kid){
    ...
    $.post("<?php echo U('del_true');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('fans');?>");
        }else{
            alert('删除失败！');
        }
    });
