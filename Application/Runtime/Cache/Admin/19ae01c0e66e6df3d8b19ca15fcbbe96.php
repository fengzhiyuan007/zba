...
<form action="javascript:;" method="post">
    <div class="tools">
        ...
        类型: <select name="pay_type" id="pay_type" onchange="sendname()">
            ...
            <option value="houtai" <?php if( $pay_type == houtai ): ?>selected<?php endif; ?>>后台</option>
            <option value="alipay_qr" <?php if( $pay_type == alipay_qr ): ?>selected<?php endif; ?>>支付宝扫码</option>
            <option value="wx_pub_qr" <?php if( $pay_type == wx_pub_qr ): ?>selected<?php endif; ?>>微信扫码</option>
        </select>
   &nbsp;&nbsp;日期： <input type="text" class="laydate-icon" name="start_time" id="start" size="20" value="<?php echo urldecode($_GET['start']) ?>" readonly> -
            <input type="text" class="laydate-icon" name="end_time" id="end" size="20" value="<?php echo ($_GET['end']); ?>" readonly>
...
<div style="float: right">总金额:<?php echo ($tongji["count_money"]); ?>&nbsp;&nbsp;&nbsp;&nbsp;总钻石:<?php echo ($tongji["meters_money"]); ?></div>
<!-----------------------------------------内容结束--------------------------------------------------->
</div>
