...
function getnums(){
    var num = $("#nus").val();
    var type = $("#type").val();
    window.location.href="/Admin/Base/message?nums="+num+"&type="+type;
}
function sendname() {
    var num = $("#nus").val();
    var type = $("#type").val();
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/Base/toadd_message">添加</a></span></div>
...
<input name="dele" type="submit" value="删除" onclick="del();" class="wr"   style="border:none; background-color:#F2F7FD; color:#2D52A5;margin-top:3px;" /></a></span></div>
   ...
   <!--&nbsp;&nbsp;类型： <select name="type" id="type" onchange="sendname();">-->
     <!--<option value="">请选择</option>-->
     <!--<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?>-->
      <!--<option value="<?php echo ($i); ?>" <?php if( $i == $lei ): ?>selected=selected<?php else: endif; ?>><?php echo ($l); ?></option>-->
         <!--<?php endforeach; endif; else: echo "" ;endif; ?>-->
    <!--</select>-->
<!--&lt;!&ndash;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" name="button" value="搜索" id="button" onclick="sendname()">&ndash;&gt;-->
<!--</span>-->
...
<tr class="tabTitleMain">
    ...
    <td width="10%" align="center">是否发送</td>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    <td align="center" class="onerow">
        <?php switch($l['state']){ case 1: echo "即时消息";break; case 2: echo "新注册消息";break; } ?>
    </td>

    <td align="center" class="onerow"><?php if( $l["state"] == 2 ): ?>--<?php elseif( $l["status"] == 1 ): ?>未发送<?php elseif( $l["status"] == 2 ): ?>已发送<?php endif; ?></td>
    <td align="center" class="onerow"><?php if( $l["state"] == 2 ): ?>--<?php elseif( $l["status"] == 1 ): ?>--<?php elseif( $l["status"] == 2 ): echo (date("Y-m-d H:i:s",$l["stime"])); endif; ?></td>
    <td align="center" class="onerow"><?php echo ($l["content"]); ?></td>

  <td align="center">
      <?php if( $l["state"] == 1 and $l["status"] == 1 ): ?><a href="<?php echo U('send',array('id'=>$l['id']));?>">发送</a> |<?php endif; ?>
      <?php if( $l["state"] == 1 and $l["status"] == 2 ): else: ?><a href="<?php echo U('toadd_message',array('id'=>$l['id']));?>">编辑</a> |<?php endif; ?>
          <a href="javascript:;"  onclick="del(<?php echo ($l["id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_message');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('message');?>");
        }else{
            alert('删除失败！');
        }
    });
