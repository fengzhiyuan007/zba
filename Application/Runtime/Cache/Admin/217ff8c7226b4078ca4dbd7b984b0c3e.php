...
function getnums(){
    ...
    window.location.href="/Admin/Caiwu/give_gift_list?nums="+num+"&start="+start+"&end="+end+"&username="+username+"&status="+status;
}
function xiazai(){
    var num = $("#nus").val();
    var status = $("#status").val();
    ...
    window.location.href="/Admin/Caiwu/give_gift_list?act="+download+"&nums="+num+"&start="+start+"&end="+end+"&username="+username+"&status="+status;
}
function sendname() {
    var num = $("#nus").val();
    var status = $("#status").val();
...
<div class="tools" style="text-align:center">
    <span style="padding-top:8px;">统计送礼钻石:<?php echo ((isset($sum) && ($sum !== ""))?($sum):0); ?>钻石</span>
    <span style="float:right;border: 1px solid #d0d0d0;padding: 0px 20px;"><a href="javascript:;" onclick="xiazai()">导出excl</a></div>
</div>
    <div class="tools">
   ...
   &nbsp;&nbsp;日期： <input type="text" class="laydate-icon" name="start_time" id="start" size="20" value="<?php echo ($start); ?>" readonly> - <input type="text" class="laydate-icon" name="end_time" id="end" size="20" value="<?php echo ($end); ?>" readonly>
        关键词: <input type="text" name="username" id="username" value="<?php echo ($username); ?>" placeholder="输入昵称、ID、礼物名称" size="30">
