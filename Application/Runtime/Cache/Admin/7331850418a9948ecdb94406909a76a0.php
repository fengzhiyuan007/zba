...
function getnums(){
    var num = $("#nus").val();
    window.location.href="/Admin/Base/gift_list?nums="+num;
}
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/Base/toadd_gift">添加</a></span></div>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["gift_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>

    <td align="center"><img src="<?php echo ($l["img"]); ?>" style="width: 50px;height: 50px;"></td>
    <td align="center" class="onerow"><?php echo ($l["name"]); ?></td>
    <td align="center" class="onerow"><?php echo ($l["price"]); ?></td>
    <td align="center" class="onerow"><?php echo ($l["experience"]); ?></td>
    <td align="center" class="onerow"><?php if( $l["is_running"] == 1 ): ?>否<?php else: ?>是<?php endif; ?></td>
    <td align="center" class="onerow"><?php if( $l["is_special"] == -1 ): ?>非特效
        <?php elseif( $l["is_special"] == 1 ): ?><span style="color:red;">汽车</span>
        <?php elseif( $l["is_special"] == 2 ): ?><span style="color:red;">轮船</span>
        <?php elseif( $l["is_special"] == 3 ): ?><span style="color:red;">飞机</span>
        <?php elseif( $l["is_special"] == 4 ): ?><span style="color:red;">豪华邮轮</span>
        <?php elseif( $l["is_special"] == 5 ): ?><span style="color:red;">城堡</span>
        <?php elseif( $l["is_special"] == 6 ): ?><span style="color:red;">丘比特</span><?php endif; ?></td>
    <td align="center" class="onerow"><?php echo (date("Y-m-d H:i",$l["intime"])); ?></td>

  <td align="center">
                  <a href="<?php echo U('toadd_gift',array('id'=>$l['gift_id']));?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                  <a href="javascript:;"  onclick="del(<?php echo ($l["gift_id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_gift');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('gift_list');?>");
        }else{
            alert('删除失败！');
        }
    });
