...
function getnums(){
    ...
    window.location.href="/Admin/Live/music?nums="+num+"&username="+username;
}
...
<div class="tools"> 
<div class="add"><span><a href="/Admin/Live/toadd_music">添加</a></span></div>
...
<input name="dele" type="submit" value="删除" onclick="del();" class="wr"   style="border:none; background-color:#F2F7FD; color:#2D52A5;margin-top:3px;" /></a></span></div>
    ...
    <span style="float:right;padding-top:5px;">
        关键词:<input type="text" name="username" id="username" value="<?php echo ($username); ?>" placeholder="输入歌名、歌手" size="25">
...
<tr class="tabTitleMain">
    ...
    <td width="" align="center">歌名</td>
    <td width="" align="center">歌手</td>
    <td width="" align="center">时长</td>
    <td width="" align="center">大小</td>
    <td width="" align="center">时间</td>
    <td width="25%" align="center">播放</td>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["music_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    <td align="center" class="onerow"><?php echo ($l["song_name"]); ?></td>
    <td align="center" class="onerow"><?php echo ($l["singer"]); ?></td>
    <td align="center" class="onerow"><?php echo ($l["time"]); ?></td>
    <td align="center" class="onerow"><?php echo ($l["size"]); ?></td>
    <td align="center" class="onerow"><?php echo (date("Y-m-d H:i",$l["intime"])); ?></td>
    <td align="center" class="onerow">
        <audio src="<?php echo ($l["path"]); ?>" controls="controls" >
        </audio>
    </td>

  <td align="center">
                  <a href="<?php echo U('toadd_music',array('id'=>$l['music_id']));?>">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
                  <a href="javascript:;"  onclick="del(<?php echo ($l["music_id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_music');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('music');?>");
        }else{
            alert('删除失败！');
        }
    });
