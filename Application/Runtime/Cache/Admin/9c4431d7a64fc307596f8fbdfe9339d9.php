...
function getnums(){
    ...
    window.location.href="/Admin/Base/feedback?nums="+num+"&username="+username;
}
...
<div class="tools"> 
<!--<div class="add"><span><a href="/Admin/Base/toadd_price">添加</a></span></div>-->
...
<input name="dele" type="submit" value="删除" onclick="del();" class="wr"   style="border:none; background-color:#F2F7FD; color:#2D52A5;margin-top:3px;" /></a></span></div>
    ...
    <span style="float:right;padding-right:50px;padding-top:5px;">
        关键词: <input type="text" name="username" id="username" value="<?php echo ($username); ?>" placeholder="输入昵称、ID、内容" size="30">
...
<tr class="tabTitleMain">
    ...
    <td width="15%" align="center">用户环信账号</td>
    <td width="25%" align="center">反馈内容</td>
    <td width="15%" align="center">时间</td>
    <td width="10%" align="center">操作</td>
</tr>


<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["feedback_id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  <td align="center"><input type="checkbox" class="deleteids" value="<?php echo ($l["feedback_id"]); ?>" name="chois[]"/><?php echo ($i); ?></td>
    <td align="center" class="onerow"><?php echo ($l["username"]); ?>(<?php echo ($l["id"]); ?>)</td>
  ...
  <td align="center">
                  <a href="javascript:;"  onclick="del(<?php echo ($l["feedback_id"]); ?>);">删除</a>
...
function del(kid){
    ...
    if(!confirm('确定删除？'))
        return false;
    $.post("<?php echo U('del_feedback');?>", {ids:kid}, function(v){
        if( v == 1 ){
            alert('删除成功！');
            location.reload("<?php echo U('feedback');?>");
        }else{
            alert('删除失败！');
        }
    });
