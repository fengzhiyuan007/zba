...
function getnums(){
    ...
    window.location.href="/Admin/Live/recorded?nums="+num+"&start="+start+"&end="+end+"&username="+username;
}
function sendname() {
    var num = $("#nus").val();
    var start = $("#start").val();
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
  ...
  <td align="center">
     <!-- <a href="javascript:;" onclick="sel(<?php echo ($l["live_store_id"]); ?>);" style="color: #0a83cd">查看</a>
      |&nbsp;&nbsp; -->
      <a href="javascript:;" onclick="dd(<?php echo ($l["live_store_id"]); ?>);" style="color: #0a83cd">查看</a>
      |&nbsp;&nbsp;
...
<script type="text/javascript">
    function dd(id){
        layer.open({
            ...
            closeBtn: 1,
            shadeClose: false,
            content: "/Admin/Live/play_m3u8?id="+id,
        });
    }
    function sel(id) {
        layer.open({
