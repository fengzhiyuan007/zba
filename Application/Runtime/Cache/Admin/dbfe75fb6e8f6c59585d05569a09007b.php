...
<script type="text/javascript">
    ...
    function getnums(){
        ...
        window.location.href="/Admin/User/live_user?nums="+num+"&start="+start+"&end="+end+"&username="+username+"&banned="+banned+"&titles="+titles;
    }
    function sendname() {
        var num = $("#nus").val();
        var start = $("#start").val();
        var end = $("#end").val();
        var username = $("#username").val();
        window.location.href="/Admin/User/live_user?nums="+num+"&start="+start+"&end="+end+"&username="+username;
    }
</script>

<form action="javascript:;" method="post">
    <div class="tools">
        <div class="add"><span><a href="/Admin/User/toadd_live_user">添加</a></span></div>
    ...
    <div class="content">
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabBox">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="tabTextMain" id="f_<?php echo ($l["id"]); ?>" onmouseout="this.style.background='#FFFFFF';" onmouseover="this.style.background='#fbf435';">
                    ...
                    <td align="center">
                        <a href="<?php echo U('toadd_live_user',array('id'=>$l['user_id']));?>">编辑</a>
...
<script type="text/javascript">
    ...
    function del(kid){
        ...
        $.post("<?php echo U('del');?>", {ids:kid}, function(v){
            if( v == 1 ){
                alert('删除成功！');
                location.reload("<?php echo U('live_user');?>");
            }else{
                alert('删除失败！');
            }
        });
